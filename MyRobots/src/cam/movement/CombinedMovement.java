...
public class CombinedMovement implements Movement {
	...
	/**
	 * Math.PI/2 would be perpendicular movement, less will keeps us moving away slightly
	 */
	...
	public void update() {
		...
		if (robot.getOthers() > 1) {
			...
			if (distanceToNextDestination < 15) {
				...
				for (int i = 0; i < 200; i++) {
					/**
					 * Calculate the testPoint somewhere around the current position. 100 + 200*Math.random() proved to
					 * be good if there are around 10 bots in a 1000x1000 field. But this needs to be limited to the
					 * distanceToTarget*0.8. This way the bot won't run into the target (should be the closest bot)
					 */
	...
	public double evaluate(Point2D.Double point, double addLast) {
		...
		while (enumeration.hasMoreElements()) {
			...
			/**
			 * - Math.min(enemy.energy/myEnergy,2) is multiplied because enemy.energy/myEnergy is an indicator of how
			 * dangerous an enemy is - Math.abs(Math.cos(calAngle(myLocation,point) - calcAngle(enemy.position,point)))
			 * is bigger if the moving direction isn't good in relation to a certain bot. It would be more natural to
			 * use Math.abs( Math.cos(calcAngle(point,myLocation) - calcAngle(enemy.poistion, myLocation))) but this
			 * wasn't going to give me good results - 1 / point.distanceSq(enemy.position) is just the normal anti
			 * gravity thing
			 */
			if (enemy.live) {
